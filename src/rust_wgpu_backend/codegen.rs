...
enum SplitPoint
{
	...
	Yield { external_function_id : ir::ExternalFunctionId, yielded_node_results : Box<[NodeResult]>, continuation_join_point_id_opt : Option<JoinPointId> },
...
impl<'program> CodeGen<'program>
{
	...
	fn compile_externally_visible_scheduling_funclet(&mut self, funclet_id : ir::FuncletId, pipeline_context : &mut PipelineContext)
	{
		...
		let mut default_join_point_id_opt = 
		{
			...
			let value_funclet_id = funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap();
		...
		while current_funclet_id_opt.is_some()
		{
			while let Some(current_funclet_id) = current_funclet_id_opt
			{
				...
				current_output_node_results = match split_point
				{
					...
					SplitPoint::Yield { external_function_id, yielded_node_results, mut continuation_join_point_id_opt } =>
					{
						...
						self.code_generator.build_yield(external_function_id, yielded_var_ids.as_slice());
	...
	fn compile_scheduling_funclet(&mut self, funclet_id : ir::FuncletId, argument_node_results : &[NodeResult], /*argument_slot_ids : &[SlotId],*/ pipeline_context : &mut PipelineContext, placement_state : &mut PlacementState, default_join_point_id_opt : &mut Option<JoinPointId>) -> SplitPoint //Box<[SlotId]>
	{
		...
		for (current_node_id, node) in funclet.nodes.iter().enumerate()
		{
			...
			match node
			{
				...
				{
					assert_eq!(funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap(), operation.funclet_id);
				...
				ir::Node::InlineJoin { funclet : funclet_id, captures, continuation : continuation_join_node_id } => 
				{
					...
					assert_eq!(join_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap(), funclet_scoped_state.value_funclet_id);
					...
					let join_point_id = placement_state.join_graph.create(JoinPoint::SimpleJoinPoint(SimpleJoinPoint{value_funclet_id : join_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap(), scheduling_funclet_id : * funclet_id, captures : captured_node_results.into_boxed_slice(), continuation_join_point_id}));
					funclet_scoped_state.node_results.insert(current_node_id, NodeResult::Join{ join_point_id });
				}
				ir::Node::SerializedJoin { funclet : funclet_id, captures, continuation : continuation_join_node_id } => 
				{
					...
					let join_point_id = placement_state.join_graph.create(JoinPoint::SerializedJoinPoint(SerializedJoinPoint{value_funclet_id : join_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap(), scheduling_funclet_id : * funclet_id, argument_ffi_types, continuation_join_point_id}));
					funclet_scoped_state.node_results.insert(current_node_id, NodeResult::Join{ join_point_id });
				}
		...
		let split_point = match & funclet.tail_edge
		{
			...
			{
				let encoded_value_funclet_id = funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap();
			...
			ir::TailEdge::Yield{external_function_id, yielded_nodes, next_funclet : next_funclet_id, continuation_join : continuation_join_node_id, arguments : argument_node_ids} =>
			{
				let continuation_join_point_id = funclet_scoped_state.move_node_join_point_id(* continuation_join_node_id).unwrap();
				let continuation_join_point = placement_state.join_graph.get_join(continuation_join_point_id);
				...
				let join_point_id = placement_state.join_graph.create(JoinPoint::SimpleJoinPoint(SimpleJoinPoint{value_funclet_id : join_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap(), scheduling_funclet_id : * next_funclet_id, captures : argument_node_results.into_boxed_slice(), continuation_join_point_id}));
				SplitPoint::Yield{external_function_id : * external_function_id, yielded_node_results : output_node_results.into_boxed_slice(), continuation_join_point_id_opt : Some(join_point_id)}
			}
			...
			ir::TailEdge::ScheduleCall { value_operation : value_operation_ref, callee_funclet_id : callee_scheduling_funclet_id_ref, callee_arguments, continuation_join : continuation_join_node_id } =>
			{
				...
				let callee_value_funclet_id = callee_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap();
			...
			ir::TailEdge::ScheduleSelect { value_operation, condition : condition_slot_node_id, callee_funclet_ids, callee_arguments, continuation_join : continuation_join_node_id } =>
			{
				...
				assert_eq!(value_operation.funclet_id, true_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap());
				assert_eq!(value_operation.funclet_id, false_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap());
			...
			ir::TailEdge::DynamicAllocFromBuffer {buffer : buffer_node_id, dynamic_allocation_size_slots, success_funclet_id, failure_funclet_id, arguments, continuation_join : continuation_join_node_id} =>
			{
				...
				assert_eq!(funclet_scoped_state.value_funclet_id, true_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap());
				assert_eq!(funclet_scoped_state.value_funclet_id, false_funclet.spec_binding.get_value_spec().funclet_id_opt.unwrap());
	...
	fn generate_pipeline(&mut self, pipeline : & ir::Pipeline)
	{
		...
		{
			let mut ffi_yield_points = Vec::<(ffi::ExternalFunctionId, code_generator::YieldPoint)>::new();
			match & self.program.native_interface.effects[effect_id.0]
			{
				ffi::Effect::Unrestricted =>
				{
					for (function_index, function) in self.program.native_interface.external_functions.iter()
					{
						...
						ffi_yield_point.resuming_types = cpu_effectful_operation.output_types.clone();
						ffi_yield_points.push((ffi::ExternalFunctionId(function_index), ffi_yield_point));
					}
				}
				ffi::Effect::FullyConnected{effectful_function_ids} =>
				{
					for & external_function_id in effectful_function_ids.iter()
					{
						let cpu_effectful_operation = self.program.native_interface.external_functions[external_function_id.0].get_cpu_effectful_operation().unwrap();
						let mut ffi_yield_point : code_generator::YieldPoint = Default::default();
						ffi_yield_point.name = cpu_effectful_operation.name.clone();
						ffi_yield_point.yielded_types = cpu_effectful_operation.input_types.clone();
						ffi_yield_point.resuming_types = cpu_effectful_operation.output_types.clone();
