...
) -> Option<asm::TypeId>
{
    match node {
        ...
        asm::Node::CallFunctionClass { function_id: Some(f_id), .. } => {
...
impl TypingContext
{
        ...
        for vf in vfs.iter_mut() {
            for com_opt in vf.0.commands.iter_mut() {
                let new_node_opt = if let Some(asm::Command::Node(nn)) = com_opt {
                    match &nn.node {
                        asm::Node::Constant { value, type_id } => type_id.clone().map(|t| {
                            ...
                            asm::Node::Constant { value: value.clone(), type_id }
                        }),
                        _ => None,
                    }
                } else {
                ...
                if let Some(new_node) = new_node_opt {
                    ...
                    let nn = asm::NamedNode { name, node: new_node };
    ...
    fn label_buffer_space(&self) -> String { "buffspace".to_string() }
}
