...
pub fn check_program(program: &super::ir::Program) -> Result<(), error::Error> {
    for (funclet_id, funclet) in program.funclets.iter() {
        let funclet_error_contextualizer =
            |writer: &mut dyn std::fmt::Write| write!(writer, "In funclet #{}", funclet_id);
        let funclet_error_context =
            error::ErrorContext::new(None, Some(&funclet_error_contextualizer));
