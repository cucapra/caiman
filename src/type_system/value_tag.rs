...
pub fn check_value_tag_compatibility_interior_branch(program : & ir::Program, value_operation : ir::RemoteNodeId, condition_value_tag : ir::ValueTag, source_value_tags : &[ir::ValueTag], destination_value_tag : ir::ValueTag)
{
	...
	{
		check_value_tag_compatibility_interior(& program, condition_value_tag, ir::ValueTag::Operation{remote_node_id : ir::RemoteNodeId{funclet_id : value_operation.funclet_id, node_id : * condition}});
	...
	for (branch_index, branch_node_id) in branch_node_ids.iter().enumerate()
	{
		...
		match source_value_tag
		{
			...
			{
				check_value_tag_compatibility_interior(& program, source_value_tag, ir::ValueTag::Operation{remote_node_id : ir::RemoteNodeId{funclet_id : value_operation.funclet_id, node_id : * branch_node_id}});
			}
			_ =>
			{
				check_value_tag_compatibility_interior(& program, source_value_tag, destination_value_tag);
			}
		}
	}
}
...
pub fn check_value_tag_compatibility_interior(program : & ir::Program, source_value_tag : ir::ValueTag, destination_value_tag : ir::ValueTag)
{
	match (source_value_tag, destination_value_tag)
	{
