...
fn check(b: bool, i: Info, e: SemanticError) 
...
pub fn context_get(
    ...
    context: &Context, 
    x: &Var,
) -> Result<Type, (Info, SemanticError)>
{
...
where
    E: HasInfo,
{
    let (param_ts, ret_r) = function_context.get(f.to_string())
        .map_err(|e| (info, e))?;
    check(
...
) -> Result<ExprType, (Info, SemanticError)>
{
    ...
    let bop_body = 
        |f: &dyn Fn(ExprType) -> Result<(), (Info, SemanticError)>, ret: ExprType| { 
