...
#[derive(Debug)]
struct PartialData {
    // a dumb distinction with the rework, but whatever
...
fn new_information(value_index: &usize) -> PartialInformation {
...
fn get_blob<'a>(value_index: &usize, context: &'a mut SchedulingContext) -> &'a ScheduleBlob {
    context
        .new_schedules
        .entry(*value_index)
...
fn get_blob_mut<'a>(
    value_index: &usize,
    context: &'a mut SchedulingContext,
) -> &'a mut ScheduleBlob {
    context
        .new_schedules
        .entry(*value_index)
...
fn get_current_blob<'a>(context: &'a mut SchedulingContext) -> &'a ScheduleBlob {
    let value_index = context.location.funclet_id;
    get_blob(&value_index, context)
}

fn get_current_blob_mut<'a>(context: &'a mut SchedulingContext) -> &'a mut ScheduleBlob {
    let value_index = context.location.funclet_id;
    get_blob_mut(&value_index, context)
}

fn get_funclet<'a>(index: &usize, context: &'a SchedulingContext) -> &'a ir::Funclet {
    context.program.funclets.get(*index).unwrap()
}

fn get_current_funclet<'a>(context: &'a SchedulingContext) -> &'a ir::Funclet {
...
fn update_current_allocated(node_id: &usize, index: &usize, context: &mut SchedulingContext) {
    // Assumes we _haven't_ added this node yet
    let blob = get_current_blob_mut(context);
    let current_node = blob.schedule.core.nodes.len();
    let entry = blob.allocated.entry(*node_id).or_insert(HashMap::new());
    entry.insert(*index, current_node);
}

fn default_tail(partial: &PartialData, context: &mut SchedulingContext) -> ir::TailEdge {
...
fn add_blob(mut blob: ScheduleBlob, context: &mut SchedulingContext) {
    let new_schedule = build_funclet(
        blob.schedule.core,
        ir::FuncletKind::ScheduleExplicit,
        context,
    );
    let new_timeline = build_funclet(blob.timeline.core, ir::FuncletKind::Timeline, context);
    let schedule_id = context.program.funclets.add(new_schedule);
    let timeline_id = context.program.funclets.add(new_timeline);
    let extra = build_extra(blob.information, &timeline_id, context);
    context
        .program
        .scheduling_funclet_extras
        .insert(schedule_id, extra);
    for location in blob.remote_update.drain(..) {
        ...
        context
            .resolved_values
            .insert(location.funclet_id, resolved);
    }
}

fn finish_funclet(tail_edge: ir::TailEdge, funclet_id: &usize, context: &mut SchedulingContext) {
...
fn add_current_funclet(tail_edge: ir::TailEdge, context: &mut SchedulingContext) {
...
fn add_schedule_node(
    ...
    context
        .resolved_schedules
        .insert(context.location.clone(), resolved);
}

fn explicate_extract_result(
    node_id: &usize,
    index: &usize,
    context: &mut SchedulingContext,
) -> bool {
    ...
    match node {
        ir::Node::CallExternalCpu {
            ...
            let output = get_external(external_function_id, context).output_types[*index];
...
fn explicate_constant(type_id: &usize, context: &mut SchedulingContext) -> bool {
...
fn explicate_node(node: &ir::Node, context: &mut SchedulingContext) -> bool {
    match node {
        ir::Node::ExtractResult { node_id, index } => {
            explicate_extract_result(node_id, index, context)
        }
        ir::Node::Constant { value, type_id } => explicate_constant(type_id, context),
        ...
        ir::Node::CallExternalCpu {
            external_function_id,
            arguments,
        } => explicate_external(external_function_id, None, arguments, context),
        ir::Node::CallExternalGpuCompute {
            external_function_id,
            dimensions,
            arguments,
        } => explicate_external(external_function_id, Some(dimensions), arguments, context),
        _ => true,
    }
}

fn explicate_nodes(nodes: &Box<[ir::Node]>, context: &mut SchedulingContext) -> bool {
    let mut unresolved = false;
    for node in &**nodes {
        if !context
            .resolved_schedules
            .contains_key(&context.location.clone())
        {
...
fn explicate_funclet(funclet: &ir::Funclet, context: &mut SchedulingContext) -> bool {
...
pub fn explicate_scheduling(program: &mut ir::Program) {
    ...
    while unresolved {
        ...
        for funclet in original.iter() {
            ...
            unresolved = explicate_funclet(funclet.1, &mut context) || unresolved;
        }
    }
