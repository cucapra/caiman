...
pub fn lower_timeline_funclets(
    ...
    program
        ...
        .filter_map(|(_info, decl)| match decl {
            ast::DeclKind::TimelineFunclet {
                name,
                input,
                output,
                statements,
            } => Some(lower_timeline_funclet(
...
fn lower_timeline_funclet(
    ...
    let mut convert_type = |t: &ast::timeline::Type| {
        asm::TypeId::Local(typing_ctx.convert_and_add_timeline_type(t.clone()))
    };
    ...
    TimelineFunclet(make_asm_funclet(
