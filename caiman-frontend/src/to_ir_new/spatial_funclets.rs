...
) -> Vec<SpatialFunclet>
{
    program
        ...
        .filter_map(|(_info, decl)| match decl {
            ast::DeclKind::SpatialFunclet { name, input, output, statements } => {
                Some(lower_spatial_funclet(typing_ctx, name, input, output, statements))
            },
            _ => None,
        })
        .collect()
...
) -> SpatialFunclet
{
    ...
    SpatialFunclet(make_asm_funclet(ir::FuncletKind::Spatial, header, nodes, tail_edge))
}
