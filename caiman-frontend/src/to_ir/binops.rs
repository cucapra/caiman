...
impl ExternalBinop
{
    ...
    )
    {
        ...
        let input_types =
            vec![typing::convert_value_type(self.input1), typing::convert_value_type(self.input2)];
...
impl Context
{
    fn new() -> Self { Self { map: HashMap::new(), function_map: HashMap::new() } }
    ...
    {
        self.map.get(x).unwrap_or_else(|| panic!("Unbound variable {}", x))
    }

    fn get_function(&self, x: &str) -> &ast::value::Type
    {
        self.function_map.get(x).unwrap_or_else(|| panic!("Unbound variable {}", x))
    }
...
)
{
    ...
    match kind {
        ...
        ast::value::ExprKind::Binop(bop, ref mut e1, ref mut e2) => {
            ...
            let ebop = ExternalBinop { input1, input2, output, binop_symbol };
