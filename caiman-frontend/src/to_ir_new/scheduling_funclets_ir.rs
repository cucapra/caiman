...
use crate::parse::ast;
use crate::parse::ast::scheduling as sch;
    ...
    fn variables_used(&self) -> Vec<String> {
        match self {
            UnsplitStmt::Let {
                ...
                | sch::FullSchedulable::CallExternal(_, args) => args
                    .into_iter()
                    .filter_map(|h| h.to_option_move())
                    .collect(),
                _ => vec![],
            },
            _ => vec![],
...
fn partition_unsplit_basic_blocks(stmts: Vec<sch::Hole<UnsplitStmt>>) -> Vec<UnsplitBasicBlock> {
    let mut blocks: Vec<UnsplitBasicBlock> = vec![UnsplitBasicBlock {
        block: vec![],
        prev: vec![],
        next: vec![],
    }];
            ...
            sch::Hole::Filled(UnsplitStmt::Let {
                e:
                    sch::Hole::Filled(Expr {
                        operation:
                            Some(Operation {
                                kind: sch::FullSchedulable::Call(_, _),
                                ..
                            }),
                        ..
                    }),
...
fn split(unsplit_funclet: UnsplitFunclet) -> Vec<SplitFunclet> {
    ...
    for i in 0..num_blocks {
        ...
        let (join, tail_edge) = if block.next.len() > 0 {
            ...
            let (join, tail_edge) = match last_expr_kind {
                sch::FullSchedulable::Call(f, _xs) => {
                    ...
                    let tail = TailEdge::ScheduleCall {
                        callee_funclet_id: f.clone(),
                    };
                    (join, tail)
                }
        ...
        split_funclets.push(SplitFunclet {
            stmts,
            inputs,
            output,
            tail_edge,
