	...
	for (current_node_id, current_node) in funclet.nodes.iter().enumerate()
	{
		let last_synchronized_submission = match current_node
		{
			...
			ir::Node::SynchronizationEvent{here_place, there_place, local_past, remote_local_past} =>
			{
				...
				if let ir::Node::SubmissionEvent{here_place : submission_here_place, there_place : submission_there_place, local_past : submission_local_past} = & funclet.nodes[* remote_local_past]
				{
			...
			_ => panic!("Node #{}: {:?} is not valid for a timeline funclet", current_node_id, current_node)
		};
