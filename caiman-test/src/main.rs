...
mod tests
{
	...
	fn test_1()
	{
		...
		let result = crate::pipelines::pipeline_1::funclet11_func(crate::pipelines::pipeline_1::Instance::new(&mut root_state, & callbacks), 1);
		assert_eq!(3, result.intermediates.0);
	}
	...
	fn test_pipeline_with_value_function()
	{
		...
		let mut root_state = caiman_rt::RootState::new(&mut device, &mut queue);
		//let result = crate::pipelines::pipeline_1::run(&mut root_state, & callbacks, 1);
		let result = crate::pipelines::pipeline_with_value_function::funclet13_func(crate::pipelines::pipeline_with_value_function::Instance::new(&mut root_state, & callbacks), 1);
		assert_eq!(2, result.intermediates.0);
	}
}