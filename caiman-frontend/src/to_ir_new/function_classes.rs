...
impl FunctionClassContext {
    pub fn get(&self, funclet_name: &str) -> Option<asm::FunctionClassId> {
        self.map
            .get(funclet_name)
            .map(|elt| asm::FunctionClassId(elt.clone()))
    }
    ...
    pub fn add_type(&mut self, fc_name: &str, input: Vec<asm::TypeId>, output: Vec<asm::TypeId>) {
        if self
            .type_map
            .insert(fc_name.to_string(), (input, output))
            .is_some()
        {
...
pub fn make(program: &ast::Program) -> (Vec<asm::FunctionClass>, FunctionClassContext) {
    ...
    for (name, functions) in ast_function_classes.into_iter() {
        for funclet_name in functions.iter() {
            let was_duplicate_insert = funclet_fc_map
                .insert(funclet_name.clone(), name.clone())
                .is_some();
            if was_duplicate_insert {
                println!(
                    "WARNING: Multiple function classes include the funclet {}",
                    funclet_name
                )
            }
        }
