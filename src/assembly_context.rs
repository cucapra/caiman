...
impl Context {
    ...
    pub fn add_cpu_funclet(&mut self, name : String) {
        ...
        self.funclet_location_map.insert(name, FuncletLocation::CpuFun);
    }

    pub fn add_gpu_funclet(&mut self, name : String) {
        self.funclet_table.push(name.clone());
        self.funclet_location_map.insert(name, FuncletLocation::GpuFun);
    }

    pub fn add_local_funclet(&mut self, name : String) {
        self.current_funclet_name = Some(name.clone());
        self.funclet_table.push(name.clone());
        self.funclet_location_map.insert(name.clone(), FuncletLocation::Local);
        self.remote_map.insert(name, new_table());
    }

    pub fn add_value_function(&mut self, name : String) {
        self.funclet_table.push(name.clone());
        self.funclet_location_map.insert(name, FuncletLocation::ValueFun);
    }

    pub fn advance_local_funclet(&mut self, name: String) {
        self.location = match self.location {
            None => Some(ir::RemoteNodeId { funclet_id : 0, node_id : 0 }),
            Some(v) => Some(ir::RemoteNodeId { funclet_id : v.funclet_id + 1, node_id : 0 })
        };
