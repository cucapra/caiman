...
impl<'program> FuncletChecker<'program>
{
	pub fn new(program : & 'program ir::Program, scheduling_funclet : & 'program ir::Funclet) -> Self
	{
		...
		let mut state = Self
		{
			...
			current_node_id : 0,
			current_timeline_tag : timeline_spec.implicit_in_tag
		};
		state.initialize();
		state
	}
	...
	{
		self.current_timeline_tag = concretize_input_to_internal_timeline_tag(& self.program, self.timeline_spec.funclet_id_opt, self.current_timeline_tag);
		...
		for (index, input_type_id) in self.scheduling_funclet.input_types.iter().enumerate()
		{
			...
			let value_tag = concretize_input_to_internal_value_tag(& self.program, self.value_spec.funclet_id_opt, self.value_spec.input_tags[index]);
			let timeline_tag = concretize_input_to_internal_timeline_tag(& self.program, self.timeline_spec.funclet_id_opt, self.timeline_spec.input_tags[index]);
			let spatial_tag = concretize_input_to_internal_spatial_tag(& self.program, self.spatial_spec.funclet_id_opt, self.spatial_spec.input_tags[index]);
		...
		{
			let (value_tag, timeline_tag, spatial_tag) = (self.value_spec.output_tags[output_index], self.timeline_spec.output_tags[output_index], self.spatial_spec.output_tags[output_index]);
	...
	fn get_funclet_value_spec<'funclet>(&self, funclet : & 'funclet ir::Funclet) -> & 'funclet ir::FuncletSpec
	{
		if let ir::FuncletSpecBinding::ScheduleExplicit{value, spatial, timeline} = & funclet.spec_binding
		{
			value
		}
		else
	...
	fn get_funclet_timeline_spec<'funclet>(&self, funclet : & 'funclet ir::Funclet) -> & 'funclet ir::FuncletSpec
	{
		if let ir::FuncletSpecBinding::ScheduleExplicit{value, spatial, timeline} = & funclet.spec_binding
		{
			timeline
	...
	fn get_funclet_spatial_spec<'funclet>(&self, funclet : & 'funclet ir::Funclet) -> & 'funclet ir::FuncletSpec
	{
		if let ir::FuncletSpecBinding::ScheduleExplicit{value, spatial, timeline} = & funclet.spec_binding
		{
			spatial
	...
	fn get_funclet_input_tags(&self, funclet : & ir::Funclet, input_index : usize) -> (ir::ValueTag, ir::TimelineTag, ir::SpatialTag)
	{
		if let ir::FuncletSpecBinding::ScheduleExplicit{value, spatial, timeline} = & funclet.spec_binding
		{
			(value.input_tags[input_index], timeline.input_tags[input_index], spatial.input_tags[input_index])
		}
		else
		{
			panic!("Does not have a ScheduleExplicit spec binding")
	...
	fn get_funclet_output_tags(&self, funclet : & ir::Funclet, output_index : usize) -> (ir::ValueTag, ir::TimelineTag, ir::SpatialTag)
	{
		if let ir::FuncletSpecBinding::ScheduleExplicit{value, spatial, timeline} = & funclet.spec_binding
		{
			(value.output_tags[output_index], timeline.output_tags[output_index], spatial.output_tags[output_index])
		}
		else
		{
			panic!("Does not have a ScheduleExplicit spec binding")
	...
	pub fn check_next_node(&mut self, current_node_id : ir::NodeId)
	{
		...
		match & self.scheduling_funclet.nodes[current_node_id]
		{
			...
			{
				assert_eq!(self.value_spec.funclet_id_opt.unwrap(), operation.funclet_id);
			...
			ir::Node::EncodeCopy { place, input, output } =>
			{
				...
				check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, source_value_tag, destination_value_tag);
			...
			{
				self.current_timeline_tag = check_next_timeline_tag_on_submit(& self.program, self.timeline_spec.funclet_id_opt, * event, self.current_timeline_tag);
			...
			{
				self.current_timeline_tag = check_next_timeline_tag_on_sync(& self.program, self.timeline_spec.funclet_id_opt, * event, self.current_timeline_tag);
				...
				let fence_encoding_timeline_event =
					...
					{
						ir::RemoteNodeId{funclet_id : self.timeline_spec.funclet_id_opt.unwrap(), node_id : * node_id}
					}
					else
					{
			...
			ir::Node::DefaultJoin =>
			{
				...
				{
					let (value_tag, timeline_tag, spatial_tag) = self.get_funclet_output_tags(self.scheduling_funclet, index);
				...
				let join_point = JoinPoint { join_kind : JoinKind::Default, in_timeline_tag : self.timeline_spec.implicit_out_tag, input_timeline_tags : timeline_tags.into_boxed_slice(), input_value_tags : value_tags.into_boxed_slice(), input_spatial_tags : spatial_tags.into_boxed_slice(), input_types : input_types.into_boxed_slice() };
	...
	fn handle_join(&mut self, join_funclet_id : ir::FuncletId, captures : &[ir::NodeId], continuation_join_node_id : ir::NodeId, join_kind : JoinKind)
	{
		...
		check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, join_funclet_timeline_spec.implicit_out_tag, continuation_join_point.in_timeline_tag);
		...
		for (capture_index, capture_node_id) in captures.iter().enumerate()
		{
			...
			let (value_tag, timeline_tag, spatial_tag) = self.get_funclet_input_tags(join_funclet, capture_index);
			...
			check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, node_value_tag, value_tag);
			check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, node_timeline_tag, timeline_tag);
		...
		{
			let (value_tag, timeline_tag, spatial_tag) = self.get_funclet_input_tags(join_funclet, input_index);
		...
		for (join_output_index, join_output_type) in join_funclet.output_types.iter().enumerate()
		{
			...
			let (value_tag, timeline_tag, spatial_tag) = self.get_funclet_output_tags(join_funclet, join_output_index);
			check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, value_tag, continuation_join_value_tags[join_output_index]);
			check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, timeline_tag, continuation_join_timeline_tags[join_output_index]);
			check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, spatial_tag, continuation_join_spatial_tags[join_output_index]);
		}

		let join_point = JoinPoint { join_kind, in_timeline_tag : join_funclet_timeline_spec.implicit_in_tag, input_timeline_tags : remaining_input_timeline_tags.into_boxed_slice(), input_value_tags : remaining_input_value_tags.into_boxed_slice(), input_spatial_tags : remaining_input_spatial_tags.into_boxed_slice(), input_types : remaining_input_types.into_boxed_slice() };
	...
	pub fn check_tail_edge(&mut self)
	{
		...
		match & self.scheduling_funclet.tail_edge
		{
			...
			{
				check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, self.current_timeline_tag, self.timeline_spec.implicit_out_tag);
				...
				for (return_index, return_node_id) in return_values.iter().enumerate()
				{
					...
					let (value_tag, timeline_tag, spatial_tag) = self.get_funclet_output_tags(self.scheduling_funclet, return_index);
					check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, node_value_tag, value_tag);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, node_timeline_tag, timeline_tag);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, node_spatial_tag, spatial_tag);
				}
			...
			ir::TailEdge::Jump { join, arguments } =>
			{
				...
				check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, self.current_timeline_tag, join_point.in_timeline_tag);
				...
				for (argument_index, argument_node_id) in arguments.iter().enumerate()
				{
					...
					check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, node_value_tag, join_value_tags[argument_index]);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, node_timeline_tag, join_timeline_tags[argument_index]);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, node_spatial_tag, join_spatial_tags[argument_index]);
				}
			}
			ir::TailEdge::ScheduleCall { value_operation : value_operation_ref, callee_funclet_id : callee_scheduling_funclet_id_ref, callee_arguments, continuation_join : continuation_join_node_id } =>
			{
				...
				let callee_funclet_value_spec = & self.get_funclet_value_spec(callee_funclet);
				let callee_funclet_timeline_spec = & self.get_funclet_timeline_spec(callee_funclet);
				let callee_value_funclet_id = callee_funclet_value_spec.funclet_id_opt.unwrap();
				...
				check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, self.current_timeline_tag, callee_funclet_timeline_spec.implicit_in_tag);
				check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, callee_funclet_timeline_spec.implicit_out_tag, continuation_join_point.in_timeline_tag);
				...
				for (argument_index, argument_node_id) in callee_arguments.iter().enumerate()
				{
					...
					let node_value_tag = self.scalar_node_value_tags[argument_node_id];
					let node_spatial_tag = self.scalar_node_spatial_tags[argument_node_id];
					let (value_tag, timeline_tag, spatial_tag) = self.get_funclet_input_tags(callee_funclet, argument_index);
					check_value_tag_compatibility_enter(& self.program, self.value_spec.funclet_id_opt, value_operation, node_value_tag, value_tag);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, node_timeline_tag, timeline_tag);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, node_spatial_tag, spatial_tag);
				}
				...
				for (callee_output_index, callee_output_type) in callee_funclet.output_types.iter().enumerate()
				{
					...
					let (value_tag, timeline_tag, spatial_tag) = self.get_funclet_output_tags(callee_funclet, callee_output_index);
					...
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, timeline_tag, continuation_join_timeline_tags[callee_output_index]);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, spatial_tag, continuation_join_spatial_tags[callee_output_index]);
				}
			}
			ir::TailEdge::ScheduleSelect { value_operation, condition : condition_slot_node_id, callee_funclet_ids, callee_arguments, continuation_join : continuation_join_node_id } =>
			{
				...
				assert_eq!(value_operation.funclet_id, true_funclet_value_spec.funclet_id_opt.unwrap());
				assert_eq!(value_operation.funclet_id, true_funclet_value_spec.funclet_id_opt.unwrap());

				assert_eq!(callee_arguments.len(), true_funclet.input_types.len());
				assert_eq!(callee_arguments.len(), false_funclet.input_types.len());

				check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, self.current_timeline_tag, true_funclet_timeline_spec.implicit_in_tag);
				check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, self.current_timeline_tag, false_funclet_timeline_spec.implicit_in_tag);
				check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, true_funclet_timeline_spec.implicit_out_tag, continuation_join_point.in_timeline_tag);
				check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, false_funclet_timeline_spec.implicit_out_tag, continuation_join_point.in_timeline_tag);
				...
				for (argument_index, argument_node_id) in callee_arguments.iter().enumerate()
				{
					...
					let (true_input_value_tag, true_input_timeline_tag, true_input_spatial_tag) = self.get_funclet_input_tags(true_funclet, argument_index);
					let (false_input_value_tag, false_input_timeline_tag, false_input_spatial_tag) = self.get_funclet_input_tags(false_funclet, argument_index);

					check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, argument_slot_value_tag, true_input_value_tag);
					check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, argument_slot_value_tag, false_input_value_tag);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, argument_slot_timeline_tag, true_input_timeline_tag);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, argument_slot_timeline_tag, false_input_timeline_tag);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, argument_slot_spatial_tag, true_input_spatial_tag);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, argument_slot_spatial_tag, false_input_spatial_tag);
				}
				...
				for (output_index, _) in true_funclet.output_types.iter().enumerate()
				{
					...
					let (true_output_value_tag, true_output_timeline_tag, true_output_spatial_tag) = self.get_funclet_output_tags(true_funclet, output_index);
					let (false_output_value_tag, false_output_timeline_tag, false_output_spatial_tag) = self.get_funclet_output_tags(false_funclet, output_index);
					check_value_tag_compatibility_interior_branch(& self.program, self.value_spec.funclet_id_opt, * value_operation, condition_value_tag, &[true_output_value_tag, false_output_value_tag], continuation_input_value_tag);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, true_output_timeline_tag, continuation_join_timeline_tags[output_index]);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, false_output_timeline_tag, continuation_join_timeline_tags[output_index]);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, true_output_spatial_tag, continuation_join_spatial_tags[output_index]);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, false_output_spatial_tag, continuation_join_spatial_tags[output_index]);
				}
			}
			ir::TailEdge::DynamicAllocFromBuffer {buffer : buffer_node_id, dynamic_allocation_size_slots : dynamic_allocation_size_node_ids, success_funclet_id, failure_funclet_id, arguments, continuation_join : continuation_join_node_id} =>
			{
				...
				assert_eq!(self.value_funclet_id, true_funclet_value_spec.funclet_id_opt.unwrap());
				assert_eq!(self.value_funclet_id, false_funclet_value_spec.funclet_id_opt.unwrap());
				...
				for (allocation_size_slot_index, allocation_size_node_id_opt) in dynamic_allocation_size_node_ids.iter().enumerate()
				{
					...
					let (true_input_value_tag, true_input_timeline_tag, true_input_spatial_tag) = self.get_funclet_input_tags(true_funclet, input_index);
					check_spatial_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, buffer_spatial_tag, true_input_spatial_tag);
				...
				for (output_index, _) in true_funclet.output_types.iter().enumerate()
				{
					...
					let (true_output_value_tag, true_output_timeline_tag, true_output_spatial_tag) = self.get_funclet_output_tags(true_funclet, output_index);
					let (false_output_value_tag, false_output_timeline_tag, false_output_spatial_tag) = self.get_funclet_output_tags(false_funclet, output_index);
					check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, true_output_value_tag, continuation_input_value_tag);
					check_value_tag_compatibility_interior(& self.program, self.value_spec.funclet_id_opt, false_output_value_tag, continuation_input_value_tag);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, true_output_timeline_tag, continuation_join_timeline_tags[output_index]);
					check_timeline_tag_compatibility_interior(& self.program, self.timeline_spec.funclet_id_opt, false_output_timeline_tag, continuation_join_timeline_tags[output_index]);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, true_output_spatial_tag, continuation_join_spatial_tags[output_index]);
					check_spatial_tag_compatibility_interior(& self.program, self.spatial_spec.funclet_id_opt, false_output_spatial_tag, continuation_join_spatial_tags[output_index]);
