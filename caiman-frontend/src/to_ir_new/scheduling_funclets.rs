        ...
        match decl_kind {
            ast::DeclKind::SchedulingImpl {
                ...
                for sf in scheduling_funclets.iter() {
                    let mut lowered_sfs =
                        lower_scheduling_funclet(function_class_ctx, typing_ctx, value_funclet, sf);
...
impl SchedulingNodeCombo {
    fn to_named_nodes(self, name: &str) -> Vec<Option<asm::NamedNode>> {
        match self {
            SchedulingNodeCombo::LocalValue {
                operation,
                storage_type,
            } => {
                ...
                let alloc_temp_node = asm::Node::AllocTemporary {
                    place,
                ...
                let alloc_temp_nn = Some(asm::NamedNode {
                    name: Some(alloc_temp_name),
                    node: alloc_temp_node,
                });
                let encode_do_nn = Some(asm::NamedNode {
                    name: Some(encode_do_name),
                    node: encode_do_node,
                });
...
fn schedule_expr_to_node_combo(
    ...
    Some(SchedulingNodeCombo::LocalValue {
        operation,
        storage_type,
    })
}
...
fn schedule_stmt_kind(
    ...
    match filled_stmt_kind {
        ...
        ast::scheduling::StmtKind::Return(x) => {
            ...
            let read_ref = asm::Node::ReadRef {
                storage_type,
                source: Some(asm::NodeId(x.clone())),
            };
            let read_ref_named = Some(asm::NamedNode {
                name: returned_variable_name(),
                node: read_ref,
            });
...
fn returned_variable_name() -> Option<asm::NodeId> {
    Some(asm::NodeId("result".to_string()))
}
...
fn lower_scheduling_funclet(
    ...
    let mut scheduling_ctx = SchedulingContext {
        map: HashMap::new(),
    };
    ...
    let tail_edge = asm::TailEdge::Return {
        return_values: Some(vec![returned_variable_name()]),
    };
    ...
    let header = asm::FuncletHeader {
        ...
        binding: asm::FuncletBinding::ScheduleBinding(asm::ScheduleBinding {
            ...
            timeline: scheduling_funclet
                .timeline_funclet
                .clone()
                .map(|s| asm::FuncletId(s)),
            spatial: scheduling_funclet
                .spatial_funclet
                .clone()
                .map(|s| asm::FuncletId(s)),
