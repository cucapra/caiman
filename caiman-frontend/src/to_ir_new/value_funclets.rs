...
impl ExprTranslation {
    fn to_named_nodes(self) -> Vec<Option<asm::NamedNode>> {
        if let ExprTranslation::NewExpr {
            name,
            pre_output,
            output,
            post_output,
...
fn translate_expr(expr: &ast::value::Expr, label: NodeLabel) -> ExprTranslation {
    ...
    match kind {
        ...
        Binop(bop, _, _) => {
            panic!(
                "At {:?}: Binary operator {:?} was not reduced to external function",
                info, bop
            )
        }
    }
