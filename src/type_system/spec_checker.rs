...
impl<'program> FuncletSpecChecker<'program> {
    ...
    pub fn update_join_node(
        &mut self,
        node_id: ir::NodeId,
        tags: &[ir::Tag],
        implicit_tag: ir::Tag,
    ) {
        self.join_nodes.insert(
            node_id,
            JoinPoint {
                implicit_tag,
                input_tags: tags.to_vec().into_boxed_slice(),
            },
        );
    }
    ...
    pub fn join(
        ...
        node_id: ir::NodeId,
        capture_node_ids: &[ir::NodeId],
        funclet_spec: &ir::FuncletSpec,
        continuation_node_id: ir::NodeId,
    ) -> Result<(), Error> {
        ...
        self.update_join_node(
            node_id,
            remaining_input_tags.as_slice(),
            funclet_spec.implicit_in_tag,
        );
    ...
    pub fn check_return(
        &mut self,
        old_error_context: &ErrorContext,
        return_value_node_ids: &[ir::NodeId],
    ) -> Result<(), Error> {
    ...
    pub fn check_interior_call(
        &mut self,
        error_context: &ErrorContext,
        continuation_node_id: ir::NodeId,
        argument_node_ids: &[ir::NodeId],
        callee_funclet_spec: &ir::FuncletSpec,
    ) -> Result<(), Error> {
        check_tag_compatibility_interior(
            error_context,
            self.spec_funclet,
    ...
    pub fn check_call(
        ...
        match operation {
                ...
                } = &self.spec_funclet.nodes[node_id]
                {
                    ...
                    match &callee_spec_funclet.spec_binding {
                        ir::FuncletSpecBinding::Value {
                            value_function_id_opt: Some(value_function_id),
                        } if *value_function_id == *function_id => {}
                        ir::FuncletSpecBinding::Timeline {
                            function_class_id_opt: Some(function_class_id),
                        } if *function_class_id == *function_id => {}
                        _ => {
                            return Err(error_context.generic_error(&format!(
                                "Callee spec funclet #{} does not implement function class #{}",
                                callee_spec_funclet_id, *function_id
                            )))
                        }
                    }
                    self.check_vertical_call(
                        error_context,
            ...
            ir::Quotient::None => self.check_interior_call(
                error_context,
                continuation_impl_node_id,
                input_impl_node_ids,
                callee_funclet_spec,
    ...
    pub fn check_choice(
        ...
        continuation_impl_node_id: ir::NodeId,
        input_impl_node_ids: &[ir::NodeId],
        choice_remaps: &[&[(ir::NodeId, ir::NodeId)]],
        choice_specs: &[&ir::FuncletSpec],
    ) -> Result<(), Error> {
    ...
    pub fn update_node_current_with_implicit(&mut self, node_id: ir::NodeId) {
        self.update_scalar_node(
            node_id,
            self.current_implicit_tag.quot,
            self.current_implicit_tag.flow,
        )
    }
    ...
    fn transition_tag_forwards(
        tag: &mut ir::Tag,
        from_spec_node_id: ir::NodeId,
        to_spec_node_id: ir::NodeId,
    ) -> Result<(), Error> {
        match tag.quot {
    ...
    pub fn transition_state_forwards(
        &mut self,
        from_spec_node_id: ir::NodeId,
        to_spec_node_id: ir::NodeId,
    ) -> Result<(), Error> {
        ...
        Self::transition_tag_forwards(
            &mut self.current_implicit_tag,
            from_spec_node_id,
            to_spec_node_id,
        )?;
        return Ok(());
    ...
    pub fn transition_state_subset_forwards(
        &mut self,
        impl_node_ids: &[ir::NodeId],
        from_spec_node_id: ir::NodeId,
        to_spec_node_id: ir::NodeId,
    ) -> Result<(), Error> {
