...
fn make_error((l, c): (usize, usize), p: ParsingError) -> LocalError {
    LocalError {
        kind: ErrorKind::SyntaxParsing(p),
...
fn parse_error_to_error<'a>(
    ...
    match err {
        ParseError::InvalidToken { location } => make_error(
        ...
        ParseError::UnrecognizedToken { token, expected } => make_error(
            ...
            ParsingError::UnrecognizedToken(format!("{:?}", token.1), format!("{:#?}", expected)),
        ),
        ParseError::ExtraToken { token } => make_error(
            ast_factory.line_and_column(token.0),
            ParsingError::ExtraToken(format!("{:?}", token.1)),
        ),
        ParseError::User { error } => make_error((0, 0), ParsingError::User(error.to_string())),
...
pub fn parse_string(buf: String, filename: &str) -> Result<ast::Program, LocalError> {
...
pub fn parse_file(filename: &str) -> Result<ast::Program, LocalError> {
