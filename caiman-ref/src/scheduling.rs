...
enum Binding
{
	Variable{ value_box : Box<dyn Any> },
...
impl<'device, 'queue> SchedulerState<'device, 'queue>
{
	fn new(device : & 'device mut wgpu::Device, queue : & 'queue mut wgpu::Queue) -> Self
	{
		...
		Self{device, queue, buffers, funclet_instances, active_command_encoder_opt : None, /*submissions,*/ fences, slots, local_logical_timestamp : LogicalTimestamp::new(), latest_gpu_synchronized_logical_timestamp : LogicalTimestamp::new()}
	}
	...
	pub fn bind_buffer(&mut self, buffer_id : BufferId, offset : usize, size : usize) -> SlotId
	{
		...
		let binding = Binding::Buffer{buffer_id, start : offset, size};
		let slot = Slot{value_id_opt : None, queue_place : Place::Gpu, queue_state : QueueState::None, time_observed, binding};
	...
	pub fn discard(&mut self, slot_ids : &[SlotId])
	{
		for & slot_id in slot_ids.iter()
		{
			let mapped_buffer_id_opt = match self.get_slot(slot_id).binding
			{
				...
				Binding::ReadOnlyMappedBuffer{buffer_id, start, size, nasty_raw_pointer} => Some(buffer_id),
				Binding::WriteableMappedBuffer{buffer_id, start, size, nasty_raw_pointer} => Some(buffer_id),
	...
	fn begin_command_encoding(&mut self)
	{
		...
		{
			let active_command_encoder = CommandEncoder{wgpu_command_encoder_opt : None, wgpu_command_buffers : vec![]};
